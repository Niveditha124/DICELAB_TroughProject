...
def geomorphic(field, par, dt):
    ...
    denom = ((par.vs * par.Rp * 0.6 * (((par.alpha * field.k_m) ** 0.5) / (par.g / np.maximum(h, par.h_min)))) ** 0.08) ### BOOKMARK ###
    ...
    for each in denom:
        for index, item in enumerate(each):
            if item == 0.0: # if it was 0 before it remains the same
                ...
                Ze5 = ((((par.alpha * field.k_m[0, index]) ** 0.5) / (par.vs * par.Rp * 0.6 * ((par.alpha * field.k_m[0, index]) ** 0.5) / (par.g / np.maximum(h, par.h_min)))) ** 0.08) ** 5
    ...
    for x in range(10):
        Ze5 = ((((par.alpha * K_new) ** 0.5) / (par.vs * par.Rp * 0.6 * ((par.alpha * K_new) ** 0.5) / (
                    par.g / np.maximum(h_new, par.h_min)))) ** 0.08) ** 5
        E = np.maximum((par.p * (1.3 * 10 ** -7)) * par.vs * Ze5 / (1 + (1.3 * 10 ** -7) / 0.3 * Ze5), 0)
